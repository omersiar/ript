...
func (r *HeartbeatRecord) IsActive(now time.Time) bool {
	if r == nil || r.LastHeartbeatAt.IsZero() {
		return false
	}
	...
	return !now.After(r.LastHeartbeatAt.Add(3 * interval))
}
...
func NewStateManager(client *Client, trackerTopic string, partitions int32, replFactor int16, loadTimeout time.Duration) *StateManager {
	return &StateManager{
		client:       client,
