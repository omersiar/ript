...
func (t *TopicTracker) Start(ctx context.Context) error {
	...
	if err == nil && snapshot != nil {
		logging.Info("Loaded previous snapshot from %v", snapshot.Timestamp)
...
func (t *TopicTracker) scanTopics(ctx context.Context) error {
	...
	for _, meta := range topicData {
		...
		for _, partID := range meta.partitions {
			...
			if oldestTimestamp.IsZero() || partInfo.Timestamp.Before(oldestTimestamp) {
				oldestTimestamp = partInfo.Timestamp
			}
			if newestTimestamp.IsZero() || partInfo.Timestamp.After(newestTimestamp) {
				newestTimestamp = partInfo.Timestamp
			}
		}

		if !oldestTimestamp.IsZero() {
			topicStatus.OldestPartitionAge = models.CalculateDuration(oldestTimestamp)
		}
		if !newestTimestamp.IsZero() {
			topicStatus.NewestPartitionAge = models.CalculateDuration(newestTimestamp)
		}
...
func buildTopicStatusFromStatePartitions(topicName string, ts time.Time, partitions map[int32]kafka.PartitionState) *models.TopicStatus {
	topicStatus := &models.TopicStatus{
		Name:       topicName,
	...
	for partID, part := range partitions {
		age := models.CalculateDuration(part.Timestamp)
		...
		if oldestTimestamp.IsZero() || part.Timestamp.Before(oldestTimestamp) {
			oldestTimestamp = part.Timestamp
		}
		if newestTimestamp.IsZero() || part.Timestamp.After(newestTimestamp) {
...
func buildPartitionInfo(partitionID int32, currentOffset int64, previous *models.PartitionInfo, now time.Time, headerTimestamp time.Time, hasHeaderTimestamp bool) *models.PartitionInfo {
	timestamp := resolvePartitionTimestamp(previous, currentOffset, now, headerTimestamp, hasHeaderTimestamp)
	age := models.CalculateDuration(timestamp)
...
func resolvePartitionTimestamp(previous *models.PartitionInfo, currentOffset int64, now time.Time, headerTimestamp time.Time, hasHeaderTimestamp bool) time.Time {
	if previous == nil || previous.Timestamp.IsZero() {
		if hasHeaderTimestamp && !headerTimestamp.IsZero() {
			return headerTimestamp.UTC()
		}
		return now
	}
...
func (t *TopicTracker) GetInstances() []models.InstanceInfo {
	...
	for _, instance := range *instancesPtr {
		...
		instance.IsActive = !now.After(instance.LastHeartbeatAt.Add(3 * interval))
